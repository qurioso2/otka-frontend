...
export async function POST(request: Request) {
  try {
    ...
    if (typeof proforma_url === 'string') { patch.proforma_url = proforma_url; patch.proforma_generated_at = new Date().toISOString(); }
    if (typeof confirmation_document_url === 'string') patch.confirmation_document_url = confirmation_document_url;
    if (status === 'submitted') patch.submitted_at = new Date().toISOString();

    const { data: orderRow, error: fetchErr } = await supabase.from('partner_orders').select('*').eq('id', order_id).maybeSingle();
