...
export async function GET() {
  ...
  const { data } = await supabase.from('products').select('slug, visible').eq('visible', true).limit(5000);
  const urls = (data || []).map((p) => `<url><loc>${base}/p/${p.slug}</loc><changefreq>daily</changefreq><priority>0.7</priority></url>`).join('');
